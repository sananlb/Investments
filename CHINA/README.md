...
ETF-proxy / benchmark для проверки состава и цен: `MCHI`.
...
Декадная динамика в `sector_valuation_dashboard.html` должна строиться по реальным valuation data на каждую якорную дату: 10-е, 20-е, 30-е / конец месяца. Если якорная дата не торговая, используется последний торговый день до нее. Исторические точки не должны рассчитываться простым масштабированием последнего `sector_score` по `MCHI`; `MCHI` используется как benchmark/source-check и вспомогательный ценовой ориентир.
...
| Якорная дата | Фактическое закрытие | MCHI close | Legacy scale, не использовать для sector_score |
|---|---|---:|---:|
...
5. Для каждой якорной даты получить реальные valuation data на последний торговый день до якоря; `MCHI` использовать только как benchmark/source-check и вспомогательный ценовой ориентир.
