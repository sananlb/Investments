...
ETF-proxy / benchmark для проверки состава и цен: `AIQ`.
...
| Якорная дата | Фактическое закрытие | AIQ close | Legacy scale, не использовать для sector_score |
|---|---|---:|---:|
...
5. Для каждой якорной даты получить реальные valuation data на последний торговый день до якоря; `AIQ` использовать только как benchmark/source-check и вспомогательный ценовой ориентир.
